...
export default function MyGuesthousePage() {
  const [guesthouses, setGuesthouses] = useState(dummyGHs); // 게스트하우스
  ...
  const tryDeleteGuesthouse = async (id) => {
    try {
      ...
      // 프론트 상태에서도 제거
      setGuesthouses((prev) => prev.filter((gh) => gh.guesthouseId !== id));
  ...
  // 객실 삭제 (지금은 프론트 상태만 갱신)
  const handleDeleteRoom = (guesthouseId, roomId) => {
    setGuesthouses((prev) =>
      prev.map((gh) =>
        gh.guesthouseId === guesthouseId
          ? { ...gh, rooms: gh.rooms.filter((r) => r.id !== roomId) }
          : gh
      )
    );
  };

  // 게스트하우스 수정 (나중에 라우팅만 바꾸면 됨)
  const handleEditGuesthouse = (guesthouseId) => {
    // TODO: 실제 편집 페이지 경로에 맞게 수정
  ...
  return (
    <div className="container">
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <div className="flex gap-2 items-center">
                        ...
                        <button
                          type="button"
                          onClick={() =>
                            handleEditGuesthouse(item.guesthouseId)
                          }
                        >
                          <img src={EditIcon} />
                        </button>
                        ...
                        <button
                          type="button"
                          onClick={() =>
                            handleDeleteGuesthouse(item.guesthouseId)
                          }
                        >
                          <img src={DeleteIcon} />
                        </button>
                          ...
                          >
                            ...
                            <div className="flex flex-1 flex-col justify-between">
                              ...
                              <button
                                type="button"
                                onClick={() =>
                                  handleDeleteRoom(item.guesthouseId, room.id)
                                }
                                className="self-end"
                              >
      ...
      <SelectModal
        ...
        items={applications} // 여기에는 '전체' 안 들어감
