...
export default function MyGuesthouseList() {
  ...
  const applications = guesthouseProfiles.filter((p) => !p.isApproved);
  const activeGuesthouses = guesthouseProfiles.filter((p) => p.isApproved);

  // Kebab Menu & Delete Modal State
  const [menuOpenId, setMenuOpenId] = useState(null);
  const [deleteTarget, setDeleteTarget] = useState(null); // { id, isApp }
  ...
  const handleEditInfo = (e, gh) => {
    e.stopPropagation();
    setMenuOpenId(null);
    setEditTargetGh(gh.originalData);
    setEditName(gh.name || "");
    setEditImagePreview(gh.photoUrl || "");
  ...
  const confirmEditProfile = async () => {
    ...
    try {
      ...
      const originalName = editTargetGh.guesthouseName || "";
      const originalImage = editTargetGh.thumbnailImg || editTargetGh.guesthouseProfileImageUrl || editTargetGh.guesthouseImageUrl || editTargetGh.profileImageUrl || editTargetGh.imgUrl || "";
      ...
      if (finalImageUrl !== originalImage) {
        payload.profileImageUrl = finalImageUrl || null;
      }
  ...
  const handleDeleteRequest = (e, gId, isApp = false) => {
    e.stopPropagation();
    setMenuOpenId(null);
    setDeleteTarget({ id: gId, isApp });
  };

  const confirmDelete = async () => {
    if (!deleteTarget) {
      setResultModal({ visible: true, title: "오류", message: "대상을 찾을 수 없습니다.", isSuccess: false });
      return;
    }
    try {
      ...
      setResultModal({ visible: true, title: "실패", message: "요청 처리에 실패했습니다. 다시 시도해주세요.\n에러: " + errorMsg, isSuccess: false });
    } finally {
  ...
  useEffect(() => {
    if (!loading && sessionReady) {
      ...
      if (!preventAutoRedirect && activeGuesthouses.length === 1 && applications.length === 0) {
        setActiveGuesthouseId(activeGuesthouses[0].id);
        navigate("/guesthouse/dashboard", { replace: true });
      }
    }
  }, [loading, sessionReady, activeGuesthouses, applications, location.state, navigate, setActiveGuesthouseId]);
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 mb-16">
                    ...
                    <>
                      ...
                      <div className="absolute top-11 right-0 w-[140px] bg-white rounded-xl shadow-[0_4px_24px_rgba(0,0,0,0.15)] border border-grayscale-200 overflow-hidden z-40 animate-in zoom-in-95 duration-150">
                        <div
                          ...
                          onClick={(e) => handleDeleteRequest(e, app.applicationId, true)}
                        >
                          신청 취소
                        </div>
                      </div>
        ...
        {activeGuesthouses.map((gh) => {
          return (
            <div
              key={`gh-${gh.id}`}
              onClick={() => handleCardClick(gh.guesthouseId)}
            ...
            >
              ...
              <div className="relative w-full pt-[70%] bg-grayscale-50 border-b border-grayscale-100 overflow-hidden">
                {gh.photoUrl ? (
                  <img src={gh.photoUrl} alt={gh.name} className="absolute inset-0 w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" />
                ) : (
                  <div className="absolute inset-0 w-full h-full flex items-center justify-center text-grayscale-300 bg-grayscale-100">
                    <span className="text-4xl font-extrabold">{gh.name?.charAt(0)}</span>
                  </div>
                )}
                {/* 운영중 뱃지 */}
                <div className="absolute top-4 left-4 inline-flex items-center gap-1.5 px-3 py-1.5 bg-white backdrop-blur-sm text-grayscale-800 rounded-lg text-sm font-extrabold shadow-sm border border-grayscale-100">
                  <CheckCircle2 className="w-4 h-4 text-primary-blue" /> {gh.statusLabel}
                </div>
                ...
                <div className="absolute top-4 right-4 z-20 w-9 h-9 border border-grayscale-200 bg-white/90 backdrop-blur-sm rounded-full flex items-center justify-center cursor-pointer hover:bg-grayscale-100 transition shadow-sm"
                  onClick={(e) => {
                    e.stopPropagation();
                    setMenuOpenId(menuOpenId === gh.id ? null : gh.id);
                  }}
                >
                  <MoreVertical className="w-5 h-5 text-grayscale-700" />
                  ...
                  {menuOpenId === gh.id && (
                    <>
                      ...
                      <div className="absolute top-11 right-0 w-[140px] bg-white rounded-xl shadow-[0_4px_24px_rgba(0,0,0,0.15)] border border-grayscale-200 overflow-hidden z-40 animate-in zoom-in-95 duration-150">
                        ...
                        <div
                          className="px-4 py-3.5 text-sm font-bold text-red-500 hover:bg-red-50 cursor-pointer transition-colors"
                          onClick={(e) => handleDeleteRequest(e, gh.applicationId, false)}
                        >
                          게스트하우스 삭제
                        </div>
                      </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="bg-white rounded-2xl w-full max-w-sm overflow-hidden shadow-2xl relative animate-in zoom-in-95 duration-200">
            ...
            <div className="p-6 pt-10 text-center">
              ...
              <h3 className="text-xl font-extrabold text-grayscale-900 mb-2">
